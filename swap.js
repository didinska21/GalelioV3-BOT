...
async function approveIfNeeded(tokenAddress) {
  const token = new ethers.Contract(tokenAddress, ERC20_ABI, wallet);
  const balance = await token.balanceOf(wallet.address);
  const tx = await token.approve(contracts.router, balance);
  await tx.wait();
}
...
async function swap(from, to) {
  ...
  const amountIn = await tokenIn.balanceOf(wallet.address);
  ...
  await approveIfNeeded(contracts[from]);
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      console.log(chalk.green(`[SUCCESS] ${from.toUpperCase()} => ${to.toUpperCase()} | Hash: ${receipt.hash}`));
      break;
    } catch (err) {
...
rl.question(chalk.blue("Input jumlah transaksi: "), async (count) => {
  ...
  for (let i = 0; i < jumlah; i++) {
    ...
    for (const route of routes) {
      await swap(route.from, route.to);
    }
  }
